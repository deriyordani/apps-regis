...
function show_question(no){
	...
	$('.question-each').load(base_url + 'examination_student/page_attempt', { js_attempt : uc_attempt, js_no : no-1 }, function(responseText, textStatus, XMLHttpRequest) {
...
function save_answer(){
	...
	if (att_code == "") {
		window.location.replace(base_url + "examination_student/home");
	}
...
function save_answer_and_redirect(){
	...
	if (q_type == 3) {
		...
		window.open(base_url+'examination_student/finish_by_time/'+att_code+'/'+q_index+'/'+match_answer+'/'+is_marks+'/'+time_running, '_self');
	}
	else {
		var answer 	= $('input[name=f_answer]:radio:checked').val();

		window.open(base_url+'examination_student/finish_by_time/'+att_code+'/'+q_index+'/'+answer+'/'+is_marks+'/'+time_running, '_self');
	}	
}
