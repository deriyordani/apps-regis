...
function list_user_question(){
	$CI =& get_instance();
	$CI->load->model('user_m');
	
	$query = $CI->user_m->get_user_quest();	
...
function list_topic($filter = NULL){
	$CI =& get_instance();
	$CI->load->model('topic_m');
	
	if ($filter != NULL) {		
		$query = $CI->topic_m->get_filtered($filter);
	}
	else {
		$query = $CI->topic_m->get_all();	
	}
...
function list_batch($filter = NULL){
    $CI =& get_instance();
    $CI->load->model('batch_m');
    
    if ($filter != NULL) {      
        $query = $CI->batch_m->get_filtered($filter);
    }
    else {
        $query = $CI->batch_m->get_all('batch_name', 'ASC');   
    }
